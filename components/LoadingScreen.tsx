...
export default function LoadingScreen({ label = "Loading your planner" }: LoadingScreenProps) {
  return (
    <div className="min-h-screen bg-background text-foreground">
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-6 md:px-10">
        <header className="pt-10">
          <div className="leading-tight">
            <div className="text-base font-semibold text-foreground">MyStudyPlanner</div>
            <div className="text-[11px] text-muted-foreground">Made by students, for students</div>
          </div>
        </header>

        <main className="flex flex-1 items-center justify-center pb-20 pt-8">
          <section className="w-full max-w-2xl text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Your school life, connected.</p>
              ...
              <div className="grid grid-cols-4 items-start gap-2">
                {steps.map((step, index) => (
                  <div key={step} className="relative flex flex-col items-center">
                    ...
                    <div className="mt-2 text-[11px] font-medium text-muted-foreground sm:text-xs">{step}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <style jsx global>{`
        ...
        @keyframes plannerNode {
          0%, 20%, 100% { opacity: 0.28; transform: scale(0.72); }
          45%, 70% { opacity: 1; transform: scale(1); }
        }
        @keyframes plannerLink {
          0%, 25% { transform: scaleX(0); opacity: 0.2; }
          55%, 80% { transform: scaleX(1); opacity: 0.8; }
